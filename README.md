...
Type system: structs, receiver functions (reminds implicit classes), interfaces, avoid interface pointers
No monads and collection API - back to `if` and `for` loops (check out `slices` package though)
...
Concurrency: goroutines - just block it, channels, wait gruops, mutexes and others from `sync` package
